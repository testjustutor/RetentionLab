...
class AuditStorage:
    ...
    def store_audit_results(meeting_id, session_id, rubric_schema, ai_result):
        ...
        try:
            ...
            evidence_quote = ai_result.get("evidence_quote", "") or ""
            raw_response = json.dumps(ai_result, default=_json_default)
                ...
                for ind_name, ind_data in indicators_data.items():
                    ...
                    cur.execute(
                        ...
                        (
                            ...
                            raw_response, oqi_score, evidence_quote,
                        ),
                    )
                    indicator_count += 1
            ...
            with get_cursor() as cur:
                cur.execute(
                    """INSERT INTO session_rubric_summary
                       (meeting_id, session_id, weighted_score_pct, gate_status, gate_failures, evidence_quote)
                       VALUES (%s,%s,%s,%s,%s,%s)
                       ON DUPLICATE KEY UPDATE
                        ...
                        evidence_quote = VALUES(evidence_quote)""",
                    (
                        meeting_id, session_id, oqi_score,
                        "all_passed" if not gate_failures else "gate_failed",
                        json.dumps(gate_failures), evidence_quote,
                    ),
                )
            log_with_type("info", f"audit/storage: summary saved for meeting={meeting_id} session={session_id} oqi={oqi_score}", "PYTHON_ENGINE")
        except Exception as e:
