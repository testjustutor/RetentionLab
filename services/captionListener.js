...
class CaptionListener {
  ...
  async start() {
    logger.info('👂 Caption listener started (Tracking Active Speakers)');
    this.intervalId = setInterval(async () => {
      try {
        await this.pollCaptions(); 
      } catch (err) {
        logger.error('Error in caption listener loop:', err.message);
      }
    }, 3000);
  }
  ...
  async stop() {
    if (this.intervalId) {
      ...
      this.intervalId = null;
      logger.info('👂 Caption listener stopped');
    }
  }
    ...
    try {
      ...
      for (const text of snippets) {
        if (!this.seenRows.has(text)) {
          ...
          logger.info(`[Transcript Content]: ${text}`);
        }
      }
    } catch (e) {
      logger.debug(`Polling skip: ${e.message}`);
    }
  }
}
