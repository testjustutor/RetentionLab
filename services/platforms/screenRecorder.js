...
class ScreenRecorder {
  constructor(storageDir, sessionId, meetingDbId) {
    ...
    // sessionId may be a temporary tracking token at construction time (the
    // real meeting_sessions row is created later, when a human is detected).
    // setSessionId() rebuilds the path once the real id exists.
  ...
  /**
   * Swap in the REAL meeting_sessions.id once it exists and rebuild the
   * output path — otherwise the recording filename would keep the bot's
   * in-memory tracking token (e.g. wait_8_123456789) instead of Sess<id>.
   */
  ...
  _buildPath() {
    ...
    // belongs to the NEXT day, e.g. "2026-09-11_00-15" for a recording made
    // at 2026-09-12 00:15 IST. All components below now come from the same
    ...
    const fileName = `SCREEN_Meet${this.meetingDbId}_Sess${this.sessionId}_${timestamp}.mp4`;
