...
class ProfessionalMeetingBot {
    ...
    async start() {
        ...
        logger.info("TeamsAdapter: Bot joined meeting. Browser session is active.");
    ...
    async transcribeAudio(filePath) {
        if (!fs.existsSync(filePath)) {
            logger.error(`TeamsAdapter: File not found for transcription: ${filePath}`);
            return null;
        }
        ...
        for (const provider of providers) {
            try {
                logger.info(`TeamsAdapter: Attempting transcription with: ${provider.toUpperCase()}`);
                ...
                logger.info(`TeamsAdapter: ${provider.toUpperCase()} Success!`);
                ...
                logger.warn(`TeamsAdapter: ${provider.toUpperCase()} failed: ${errorMsg}. Continuing to next provider...`);
            }
        }

        // 6. Only reach here if EVERY provider in the list failed
        logger.error("TeamsAdapter: ALL transcription providers failed. Saving audio for manual retry.");
        return null;
    }
    ...
    async generateSummary(audioText, labeledText) {
        ...
        for (const provider of providers) {
            try {
                logger.info(`TeamsAdapter: Generating Summary with: ${provider.toUpperCase()}`);
                ...
                logger.warn(`TeamsAdapter: Summary failed with ${provider}, trying next...`);
            }
        }
