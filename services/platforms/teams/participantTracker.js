...
class ParticipantTracker {
    ...
    try {
      ...
      if (this.trackedParticipants.has(participantName)) {
        ...
        if (tracked.status === 'left') {
          logger.info(
            `TeamsAdapter(participantTracker): Participant rejoining - ${participantName}`
          );
          ...
          logger.debug(
            `TeamsAdapter(participantTracker): Participant already joined - ${participantName}`
          );
          return {
            success: true,
            participantName,
      ...
      logger.info(
        `TeamsAdapter(participantTracker): Participant first join - ${participantName}`
      );
      ...
      logger.error(
        `TeamsAdapter(participantTracker): Error handling participant join - ${participantName}:`,
        err
      );
      return {
        success: false,
  ...
  async handleParticipantLeave(participantName, leaveTime = new Date()) {
    try {
      ...
      if (!participantName || typeof participantName !== 'string') {
        logger.warn(
          `TeamsAdapter(participantTracker): Invalid leave event (missing name)`
        );
      ...
      if (!tracked) {
        logger.warn(
          `TeamsAdapter(participantTracker): Missing join state, auto-creating participant record - ${participantName}`
        );
        ...
        try {
          ...
          logger.error(
            `TeamsAdapter(participantTracker): Auto-recovery failed - ${participantName}`,
            err
          );
          return {
            success: false,
      ...
      if (tracked.status !== 'joined') {
        logger.debug(
          `TeamsAdapter(participantTracker): Participant already left - ${participantName}`
        );
        return {
          success: true,
          participantName,
      ...
      logger.info(
        `TeamsAdapter(participantTracker): Participant leaving - ${participantName} (rejoin: ${isRejoin})`
      );
      ...
      logger.error(
        `TeamsAdapter(participantTracker): Error handling participant leave - ${participantName}:`,
        err
      );
      return {
        success: false,
  ...
  reset() {
    ...
    logger.info(
      `TeamsAdapter(participantTracker): Tracker reset for meeting ${this.meetingId}`
    );
  }
}
