...
class CaptionListener {
  ...
  async start() {
    logger.info('TeamsAdapter(captionListener): Caption listener started (Tracking Active Speakers)');
    this.intervalId = setInterval(async () => {
      try {
        await this.pollCaptions(); 
      } catch (err) {
        logger.error('TeamsAdapter(captionListener): Error in caption listener loop:', err.message);
      }
    }, 3000);
  }
  ...
  async stop() {
    if (this.intervalId) {
      ...
      this.intervalId = null;
      logger.info('TeamsAdapter(captionListener): Caption listener stopped');
    }
  }
    ...
    try {
      ...
      for (const text of snippets) {
        if (!this.seenRows.has(text)) {
          ...
          logger.info(`TeamsAdapter(captionListener): [Transcript Content]: ${text}`);
        }
      }
    } catch (e) {
      logger.debug(`TeamsAdapter(captionListener): Polling skip: ${e.message}`);
    }
  }
}
