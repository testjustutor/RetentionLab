...
class CaptionMonitor {
  ...
  initStorage() {
    try {
      ...
      logger.info(`GoogleMeetAdapter(captionMonitor): File Created: storage/transcripts/${this.fileName}`);
      if (this.sessionId) {
        TranscriptModel.saveTranscriptFile(this.sessionId, this.fileName)
          .catch(err => logger.error(`GoogleMeetAdapter(captionMonitor): Error saving transcript file metadata: ${err.message}`));
      }
    } catch (err) {
      logger.error(`GoogleMeetAdapter(captionMonitor): Failed to initialize transcript file: ${err.message}`);
    }
  }
  ...
  async pollCaptions() {
    ...
    try {
      ...
      const hasEndPhrase = endPhrases.some(
      ...
      if (hasEndPhrase || !hasActiveMeetingPage) {
      ...
      const result = await this.getMeetTranscript();

      if (result && result.count > 0) {
        await this.processAndSaveTranscript(
          result.data
        );
      }
  ...
  async processAndSaveTranscript(transcriptData) {
    for (const item of transcriptData) {
      ...
      const previousSpeaker = this.lastKnownSpeaker;
      const previousText = this.lastSavedText || '';

      if (
        cleanName === previousSpeaker
      ) {

        const previous = previousText.toLowerCase();
        const current = cleanText.toLowerCase();
      ...
      const key = `${cleanName}:${normalizedText}`;
      ...
      if (!this.seenRows.has(key)) {
        ...
        const time = item.time || new Date().toLocaleTimeString('en-GB', { hour12: false });

        const logLine =
          `[${time}] ${cleanName}: ${cleanText}`;

        const finalLine = logLine.replace(/\s+/g, ' ').trim();
  
        logger.info(`GoogleMeet Transcript: ${finalLine}`);
        ...
        try {
          ...
          logger.error(
            `GoogleMeetAdapter(captionMonitor): File Append Error: ${fileErr.message}`
          );
        }
      }
    }
  ...
  async handleMeetingEnd() {
    ...
    logger.info("GoogleMeetAdapter(captionMonitor): [AUDIO] Initiating Audio Creation/Processing...");
    ...
    try {
        ...
        logger.info(`GoogleMeetAdapter(captionMonitor): File Finalized: ${this.fileName}`);
    } catch (err) {
        logger.error(`GoogleMeetAdapter(captionMonitor):Error finalizing file: ${err.message}`);
    }

    if (this.sessionId) {
      TranscriptModel.updateSessionEnd(this.sessionId)
        .then(() => logger.info(`GoogleMeetAdapter(captionMonitor): Session end recorded for session ${this.sessionId}`))
        .catch(err => logger.error(`GoogleMeetAdapter(captionMonitor): Error recording session end: ${err.message}`));
    }
    ...
    if (typeof this.onMeetingEnd === 'function') {
      try {
        ...
        logger.error(`GoogleMeetAdapter(captionMonitor): Meeting shutdown callback error: ${err.message}`);
      }
    }
    ...
    try {
        ...
        await browser.close();
        logger.info("GoogleMeetAdapter(captionMonitor): Bot successfully left the ended meeting.");
    } catch (e) {
        logger.error(`GoogleMeetAdapter(captionMonitor): Browser close error: ${e.message}`);
    }
  }
}
