...
class CaptionMonitor {
      ...
      captionSelectors.forEach(selector => {
        ...
        blocks.forEach(block => {
          ...
          try {
            ...
            const spans = Array
              .from(block.querySelectorAll('span'))
  ...
  async processAndSaveTranscript(
    transcriptData
  ) {
    ...
    for (const item of transcriptData) {
      ...
      if (!this.seenRows.has(key)) {
        ...
        const logLine =
          `[${timestamp}] ${cleanName}: ${cleanText}`;

        logger.info(
          `GoogleMeet Transcript: ${logLine}`
        );
  ...
  async handleMeetingEnd() {
    ...
    try {
      ...
      logger.info(
        `GoogleMeetAdapter(captionMonitor): File Finalized: ${this.fileName}`
      );

    } catch (err) {

      logger.error(
        `GoogleMeetAdapter(captionMonitor):Error finalizing file: ${err.message}`
      );
    }
