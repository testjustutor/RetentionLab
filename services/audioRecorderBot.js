...
class ProfessionalMeetingBot {
    ...
    async start() {
        ...
        logger.info("DefaultAdapter(audioRecorderBot): Bot joined meeting. Browser session is active.");
    ...
    async generateSummary(audioText, labeledText) {
        ...
        for (const provider of providers) {
            try {
                logger.info(`DefaultAdapter(audioRecorderBot): Generating Summary with: ${provider.toUpperCase()}`);
                ...
                logger.warn(`DefaultAdapter(audioRecorderBot): Summary failed with ${provider}, trying next...`);
            }
        }
