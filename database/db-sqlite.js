...
const migrateDB = async () => {
    try {
        ...
        if (!userTableInfo.some(col => col.name === 'is_active')) {
            await runAsync("ALTER TABLE users ADD COLUMN is_active INTEGER NOT NULL DEFAULT 1");
            console.log('[DB] Added is_active column to users');
        }
        if (!userTableInfo.some(col => col.name === 'is_deleted')) {
            await runAsync("ALTER TABLE users ADD COLUMN is_deleted INTEGER NOT NULL DEFAULT 0");
            console.log('[DB] Added is_deleted column to users');
        }
        if (!userTableInfo.some(col => col.name === 'updated_by')) {
            await runAsync("ALTER TABLE users ADD COLUMN updated_by INTEGER");
            console.log('[DB] Added updated_by column to users');
        }
        if (!userTableInfo.some(col => col.name === 'deleted_by')) {
            await runAsync("ALTER TABLE users ADD COLUMN deleted_by INTEGER");
            console.log('[DB] Added deleted_by column to users');
        }
        ...
        // ─── NEW: add section and color columns to header_menu_items ─────────
        const menuItemsTableInfo = await allAsync("PRAGMA table_info(header_menu_items)");
        if (!menuItemsTableInfo.some(col => col.name === 'section')) {
            await runAsync("ALTER TABLE header_menu_items ADD COLUMN section TEXT DEFAULT 'main'");
            console.log('[DB] Added section column to header_menu_items');
        }
        if (!menuItemsTableInfo.some(col => col.name === 'color')) {
            await runAsync("ALTER TABLE header_menu_items ADD COLUMN color TEXT DEFAULT 'violet'");
            console.log('[DB] Added color column to header_menu_items');
        }
        ...
        `);
        await runAsync(`CREATE INDEX IF NOT EXISTS idx_departments_company ON departments(company_id)`);
        await runAsync(`CREATE INDEX IF NOT EXISTS idx_dept_members_dept ON department_members(department_id)`);
        await runAsync(`CREATE INDEX IF NOT EXISTS idx_dept_members_user ON department_members(user_id)`);

        // ─── Users table indexes ─────────────────────────────────────────────
        await runAsync(`CREATE INDEX IF NOT EXISTS idx_users_company_id ON users(company_id)`);
        await runAsync(`CREATE INDEX IF NOT EXISTS idx_users_role_id ON users(role_id)`);
        await runAsync(`CREATE INDEX IF NOT EXISTS idx_users_status ON users(status)`);
        await runAsync(`CREATE INDEX IF NOT EXISTS idx_users_created_by ON users(created_by)`);
        // Add email verification and reset columns if missing
        const usersTableInfo = await allAsync('PRAGMA table_info(users)');
        if (!usersTableInfo.some(col => col.name === 'email_verified')) {
            await runAsync('ALTER TABLE users ADD COLUMN email_verified INTEGER NOT NULL DEFAULT 0');
            console.log('[DB] Added email_verified column to users');
        }
        if (!usersTableInfo.some(col => col.name === 'email_verified_at')) {
            await runAsync('ALTER TABLE users ADD COLUMN email_verified_at DATETIME');
            console.log('[DB] Added email_verified_at column to users');
        }
        if (!usersTableInfo.some(col => col.name === 'email_verification_token')) {
            await runAsync('ALTER TABLE users ADD COLUMN email_verification_token TEXT');
            console.log('[DB] Added email_verification_token column to users');
        }
        if (!usersTableInfo.some(col => col.name === 'email_verification_expires_at')) {
            await runAsync('ALTER TABLE users ADD COLUMN email_verification_expires_at DATETIME');
            console.log('[DB] Added email_verification_expires_at column to users');
        }
        if (!usersTableInfo.some(col => col.name === 'password_reset_token')) {
            await runAsync('ALTER TABLE users ADD COLUMN password_reset_token TEXT');
            console.log('[DB] Added password_reset_token column to users');
        }
        if (!usersTableInfo.some(col => col.name === 'password_reset_expires_at')) {
            await runAsync('ALTER TABLE users ADD COLUMN password_reset_expires_at DATETIME');
            console.log('[DB] Added password_reset_expires_at column to users');
        }
        // Create email_verified index only after the column exists
        await runAsync(`CREATE INDEX IF NOT EXISTS idx_users_email_verified ON users(email_verified)`);
        await runAsync(`CREATE UNIQUE INDEX IF NOT EXISTS idx_users_email_verification_token ON users(email_verification_token) WHERE email_verification_token IS NOT NULL`);
        await runAsync(`CREATE UNIQUE INDEX IF NOT EXISTS idx_users_password_reset_token ON users(password_reset_token) WHERE password_reset_token IS NOT NULL`);
        // Partial unique index: email must be unique only among non-deleted users.
        // This allows soft-deleting a user and creating a new one with the same email.
        await runAsync(`DROP INDEX IF EXISTS idx_users_email_unique_active`);
        await runAsync(`CREATE UNIQUE INDEX IF NOT EXISTS idx_users_email_unique_active ON users(email) WHERE deleted_at IS NULL`);
        console.log('[DB] Users table indexes created');
