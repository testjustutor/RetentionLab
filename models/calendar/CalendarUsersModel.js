...
class CalendarUsersModel {
  ...
  static async createOrUpdateUser(userId, tokens) {
   ...
   */
  static async getAllUsers({ createdBy = null, roles = null, excludeSelf = false, adminId = null } = {}) {
    return new Promise((resolve, reject) => {
      const conditions = [];
      const params = [];
      ...
      const sql = `
        SELECT calendar_integrations.*, users.id AS user_id_ref, users.email, users.role_id, roles.role_name
