...
class SessionMetadataModel {
  static upsert(metadata) {
    return new Promise((resolve, reject) => {
      const sql = `INSERT INTO session_metadata (meeting_id, student_grade, curriculum, student_location, subject, topic, session_objective, session_type, teacher_user_id, student_name, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) ON CONFLICT(meeting_id) DO UPDATE SET student_grade=excluded.student_grade, curriculum=excluded.curriculum, student_location=excluded.student_location, subject=excluded.subject, topic=excluded.topic, session_objective=excluded.session_objective, session_type=excluded.session_type, teacher_user_id=excluded.teacher_user_id, student_name=excluded.student_name, updated_at=CURRENT_TIMESTAMP`;
