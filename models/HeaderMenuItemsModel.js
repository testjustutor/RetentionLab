...
class HeaderMenuItemsModel {
  static listByRole(roleId) {
    return new Promise((resolve, reject) => db.all('SELECT * FROM header_menu_items WHERE role_id = ? AND deleted_at IS NULL ORDER BY display_order ASC', [roleId], (err, rows) => err ? reject(err) : resolve(rows || [])));
  }
