...
class MasterRubricModel {
  ...
  static createIndicator(indicator) {
    return new Promise((resolve, reject) => {
      ...
      // FIX: ported from models/super_admin/rubrics/MasterRubricModel.js, which
      // had this and this copy didn't - both copies read/write the SAME
      // rubric_indicators table (migration 056_add_rubric_calculation_fields.js
      // added requires_calculation/calculation_config to it), so an indicator
      // created via the top-level (admin) route was silently unable to set
      // calculation-driven scoring, while the super_admin route could.
  ...
  static _applyIndicatorUpdate(idOrCode, updates, resolve, reject) {
    this._indicatorByIdentifier(idOrCode).then((row) => {
      ...
      // FIX: ported alongside createIndicator's fix above, for the same reason.
