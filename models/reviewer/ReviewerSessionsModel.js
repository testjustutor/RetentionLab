...
class ReviewerSessionsModel {
  ...
  /**
   * Meetings assigned to the reviewer (for the session-page filter dropdown).
   * @param {number} reviewerId
   * @returns {Promise<Array>} [{ meeting_id, meeting_title }]
   */
  static getMeetingsForReviewer(reviewerId) {
    return new Promise((resolve, reject) => {
      const sql = `
        SELECT DISTINCT m.id AS meeting_id, m.title AS meeting_title
        FROM meetings m
        INNER JOIN meeting_reviewers mr ON mr.meeting_id = m.id AND mr.reviewer_id = ?
      ...
      db.all(sql, [reviewerId], (err, rows) => {
        if (err) {
          logger.error('[ReviewerSessionsModel] getMeetingsForReviewer error', err);
          return reject(err);
        }
        resolve(rows || []);
      });
  ...
  /**
   * Sessions inside meetings assigned to the reviewer (for the session filter dropdown).
   * @param {number} reviewerId
   * @returns {Promise<Array>} [{ session_id, meeting_id, session_status, start_time }]
   */
  static getSessionsForReviewer(reviewerId) {
    return new Promise((resolve, reject) => {
      ...
      db.all(sql, [reviewerId], (err, rows) => {
        if (err) {
          logger.error('[ReviewerSessionsModel] getSessionsForReviewer error', err);
          return reject(err);
        }
        resolve(rows || []);
      });
  ...
  /**
   * Session-level rows for the reviewer's assigned meetings, filtered by
   * date range / meeting / session / search.
   * @param {number} reviewerId
   * @param {Object} filters { from_date?, to_date?, meeting_id?, session_id?, search? }
   * @returns {Promise<{ rows: Array, summary: Object }>}
   */
  static getFilteredSessions(reviewerId, filters = {}) {
    return new Promise((resolve, reject) => {
      ...
      if (filters.to_date) {
        conditions.push('ss.start_time < DATE_ADD(?, INTERVAL 1 DAY)');
        params.push(`${filters.to_date} 00:00:00`);
      }
      if (filters.search) {
        conditions.push('(m.title LIKE ? OR m.platform LIKE ?)');
      ...
      const sql = `
        SELECT ss.id AS session_id,
               ...
               ma.video_path,
               ma.oqi_score,
               (SELECT COUNT(*) FROM meeting_session_scores ms
                WHERE ms.meeting_id = ss.meeting_id AND ms.session_id = ss.id) AS score_count,
               (SELECT AVG(ms.score) FROM meeting_session_scores ms
                WHERE ms.meeting_id = ss.meeting_id AND ms.session_id = ss.id) AS avg_score,
               (SELECT COUNT(*) FROM participants p WHERE p.meeting_id = ss.meeting_id) AS participant_count
        FROM meeting_sessions ss
        JOIN meetings m ON m.id = ss.meeting_id
        JOIN meeting_reviewers mr ON mr.meeting_id = m.id AND mr.assigned_by IS NOT NULL AND mr.assigned_by <> mr.reviewer_id
        LEFT JOIN meeting_assets ma ON ma.meeting_id = ss.meeting_id
      ...
      db.all(sql, params, (err, rows) => {
        if (err) {
          logger.error('[ReviewerSessionsModel] getFilteredSessions error', err);
          return reject(err);
        }
        const all = rows || [];
        const counts = {
          total: all.length,
          completed: all.filter((r) => (r.session_status || r.meeting_status) === 'completed').length,
          in_progress: all.filter((r) => ['in_progress', 'active', 'joining'].includes(r.session_status || r.meeting_status)).length,
          scheduled: all.filter((r) => ['queued', 'launching', 'starting'].includes(r.session_status || r.meeting_status)).length
        };
  ...
  /**
   * Session-level review items for the reviewer's evaluations page, filtered by
   * date / meeting / session. Returns one row per session inside a reviewer's
   * admin-assigned meetings (like the sessions page), each with the review status.
   * @param {number} reviewerId
   * @param {Object} filters { from_date?, to_date?, meeting_id?, session_id? }
   * @returns {Promise<{ rows: Array, summary: Object }>}
   */
  static getReviewItems(reviewerId, filters = {}) {
    return new Promise((resolve, reject) => {
      const conditions = ['mr.reviewer_id = ?', 'mr.assigned_by IS NOT NULL', 'mr.assigned_by <> mr.reviewer_id'];
      const params = [reviewerId];
      ...
      const sql = `
        ...
        WHERE ${conditions.join(' AND ')}
        ORDER BY ss.start_time DESC, ss.id DESC
        LIMIT 2000
      `;

      db.all(sql, params, (err, rows) => {
        if (err) {
          logger.error('[ReviewerSessionsModel] getReviewItems error', err);
          return reject(err);
        }
        const all = rows || [];
        const counts = {
          total: all.length,
          pending: all.filter((r) => r.review_status === 'pending' || r.review_status === 'unassigned').length,
