 ...
 */
const ReviewerSessionsModel = require('../../models/reviewer/ReviewerSessionsModel');

function ok(data, msg) { return { success: true, message: msg || null, ...(data || {}) }; }
function err(msg, code) { return { success: false, error: msg, statusCode: code || 500 }; }

// Convert an absolute stored path to a web-relative URL (shared by session mappings)
function toUrl(p) {
  if (!p) return null;
...
const controller = {
  ...
  /** GET /api/reviewer-sessions/filter-options — Meetings + sessions for the reviewer's session filters */
  async getFilterOptions(req) {
    try {
      const reviewerId = req.user.id;
      const meetings = await ReviewerSessionsModel.getMeetingsForReviewer(reviewerId);
      const sessions = await ReviewerSessionsModel.getSessionsForReviewer(reviewerId);
      return ok({ meetings, sessions });
    } catch (e) { return err(e.message); }
  },

  /** GET /api/reviewer-sessions/filtered-sessions — Sessions filtered by date/meeting/session/search */
  async getFilteredSessions(req) {
    try {
      const reviewerId = req.user.id;
      const q = req.query || {};
      const result = await ReviewerSessionsModel.getFilteredSessions(reviewerId, {
  ...
  /** GET /api/reviewer-sessions/evaluations — Session-level review items for the evaluations page, filtered */
  async getEvaluations(req) {
    try {
      const reviewerId = req.user.id;
      const q = req.query || {};
      const result = await ReviewerSessionsModel.getReviewItems(reviewerId, {
        from_date: q.from_date || '',
        to_date: q.to_date || '',
        meeting_id: q.meeting_id || '',
        session_id: q.session_id || ''
      ...
      return ok({ items, counts: result.summary });
    } catch (e) { return err(e.message); }
  },
