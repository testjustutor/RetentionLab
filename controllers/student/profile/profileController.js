/**
 * controllers/student/profile/profileController.js
 * Student profile logic — only calls models. Self-service only (a student
 * may view/update their own profile; there is no admin-on-behalf-of path here).
 * Mirrors controllers/super_admin/profile/profileController.js, adapted so a
 * non-admin caller does not trip UsersModel's admin-only guard (see update()).
 */
...
const controller = {
  /** GET /api/student/profile/me — current logged-in user */
  async me(req) {
    try {
  ...
  /** POST /api/student/profile/change-password — change own password */
  ...
  /** PUT /api/student/profile/:id — update own fields (self only) */
  async update(req) {
    try {
