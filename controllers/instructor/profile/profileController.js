/**
 * controllers/instructor/profile/profileController.js
 * Instructor profile logic — only calls models. Self-service only (an instructor
 * may view/update their own profile; there is no admin-on-behalf-of path here).
 * Mirrors controllers/reviewer/profile/profileController.js / controllers/student/profile/profileController.js.
 */
...
const controller = {
  /** GET /api/instructor/profile/me — current logged-in user */
  async me(req) {
    try {
  ...
  /** POST /api/instructor/profile/change-password — change own password */
  ...
  /** PUT /api/instructor/profile/:id — update own fields (self only) */
  async update(req) {
    try {
