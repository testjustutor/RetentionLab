...
function setActiveFolder(btnElement) {
    ...
    document.querySelectorAll('.sidebar-btn').forEach(b => {
        b.className = "sidebar-btn w-full text-left flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-teal-100 text-teal-700 text-xs font-medium transition cursor-pointer";
        const iconContainer = b.querySelector('svg');
        if (iconContainer) iconContainer.className.baseVal = "w-4 h-4 text-teal-700 shrink-0";
    });

    // Set active specific classes
    btnElement.className = "sidebar-btn w-full text-left flex items-center gap-2 px-3 py-2 rounded-lg bg-teal-600/20 text-teal-800 border border-teal-500 text-xs font-medium";
    const icon = btnElement.querySelector('svg');
    if (icon) icon.className.baseVal = "w-4 h-4 text-teal-700 shrink-0";
...
async function loadAssetsData(folderName) {
    ...
    // Set loading state
    grid.innerHTML = '<div class="col-span-full py-8 text-center text-teal-700 text-sm">Loading assets...</div>';

    try {
        // Attempt to fetch from real API backend
        const apiUrl = '/api/super_admin/content/assets/folder/' + folderName;
        ...
        const grid = document.getElementById('assetsGrid');
        grid.innerHTML = `
          <div class="col-span-full py-8 text-center text-rose-600 text-sm">
            Failed to load directory contents.<br>
            <span class="text-xs text-rose-700">${err.message}</span>
          </div>
        `;
    }
}
...
function renderAssets(files, folderName) {
    ...
    if (!files || files.length === 0) {
        grid.innerHTML = '<div class="col-span-full py-8 text-center text-teal-700 text-sm">No files in this directory.</div>';
        return;
    }
    ...
    files.forEach(file => {
        ...
        if (ext === '.mp3') {
            ...
            iconColorClass = 'text-amber-500/70 group-hover:text-amber-800';
        ...
        html += `
            <div class="bg-white/70 rounded-xl border-2 border-teal-300 p-3 ${activeBorder} transition cursor-pointer group flex flex-col asset-item" data-name="${escapeHtml(file.name)}" data-folder="${escapeHtml(folderName)}" data-ext="${escapeHtml(file.ext || '')}">
                <div class="w-full aspect-video bg-cyan-100 rounded-lg border-2 border-teal-300 flex flex-col items-center justify-center mb-3 ${frameClass} transition relative overflow-hidden">
                    <div class="absolute inset-0 opacity-0 transition ${bgColorClass}"></div>
                    <div class="${iconColorClass}">
                        ${iconHtml}
                    </div>
                    <span class="font-mono text-[9px] px-2 py-0.5 rounded bg-white text-teal-700 border border-teal-300">${file.ext}</span>
                </div>
                <div class="flex justify-between items-start mb-1 h-8">
                    <span class="text-[11px] font-bold text-cyan-950 leading-tight line-clamp-2" title="${escapeHtml(file.name)}">${escapeHtml(file.name)}</span>
                </div>
                <div class="flex justify-between items-center mt-auto pt-2 border-t border-teal-300">
                    <span class="text-[9px] text-teal-700 font-mono">${escapeHtml(file.size)}</span>
                    <span class="text-[9px] text-teal-700 truncate ml-1">${escapeHtml(file.type)}</span>
                </div>
            </div>
        `;
    });
...
function openFileViewer(folderName, fileName, ext) {
    ...
    // Reset content
    inner.innerHTML = '<div class="p-4 text-sm text-teal-700">Loading...</div>';
    viewer.classList.remove('hidden');

    const url = `/api/super_admin/content/assets/folder/${encodeURIComponent(folderName)}/file/${encodeURIComponent(fileName)}`;
    ...
    if (lower === '.mp3' || lower === '.wav' || lower === '.m4a' || lower === '.ogg') {
        inner.innerHTML = `
            <div class="p-4">
                <div class="mb-3 text-sm font-semibold text-cyan-950">${escapeHtml(fileName)}</div>
        ...
        .then(payload => {
            ...
            if (mime.includes('application/json') || (ext || '').toLowerCase() === '.json') {
                try {
                    ...
                    inner.innerHTML = `<pre class=\"p-4 text-sm text-cyan-950 overflow-auto\">${escapeHtml(JSON.stringify(parsed, null, 2))}</pre>`;
                } catch (e) {
                    inner.innerHTML = `<pre class=\"p-4 text-sm text-cyan-950 overflow-auto\">${escapeHtml(content)}</pre>`;
                }
                return;
            }
