...
async function loadAuditData() {
    try {
        const response = await fetch('/api/super_admin/monitoring/audit');
...
function renderAuditLogs(logs) {
    ...
    pageItems.forEach(log => {
        let lvlColor = 'text-rose-700 bg-rose-100 border-rose-300';
        if (log.level === 'INFO') lvlColor = 'text-blue-700 bg-blue-100 border-blue-300';
        else if (log.level === 'WARN') lvlColor = 'text-amber-800 bg-amber-100 border-amber-300';
        else if (log.level === 'CRITICAL') lvlColor = 'text-fuchsia-700 bg-fuchsia-100 border-fuchsia-300';

        html += `
            <tr class="hover:bg-violet-100 transition-colors">
                <td class="px-4 py-2.5 text-violet-700">${escapeHtml(log.timestamp)}</td>
                ...
                <td class="px-4 py-2.5 text-violet-800 font-semibold">${escapeHtml(log.module)}</td>
                <td class="px-4 py-2.5 text-violet-900 opacity-90 truncate max-w-sm">${escapeHtml(log.description)}</td>
                <td class="px-4 py-2.5 text-violet-700">${escapeHtml(log.user)}</td>
            </tr>
        `;
    });
