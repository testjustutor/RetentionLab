...
async function loadRecentActivity() {
    ...
    try {
        const response = await fetch('/api/audit?limit=20', { credentials: 'include' });
        ...
        if (!logs || logs.length === 0) {
            tbody.innerHTML = `<tr><td colspan="3" class="py-3 px-2 text-center text-slate-500 text-[10px]">No recent activity found</td></tr>`;
            return;
        }
        ...
        recent.forEach(log => {
            ...
            let eventColor = 'text-violet-400';
            let eventIcon = '●';
                ...
                <tr class="hover:bg-slate-800/30 transition">
                    <td class="py-1.5 px-2">
                        <div class="flex items-center gap-1.5">
                            ...
                            <span class="text-slate-400 text-[9px]">${shortDesc}</span>
                        </div>
                    </td>
                    <td class="py-1.5 px-2 text-slate-300 text-[10px] font-mono">${log.user || 'system'}</td>
                    <td class="py-1.5 px-2 text-slate-500 text-[10px] whitespace-nowrap">${timeAgo}</td>
                </tr>
            `;
        });
        ...
        tbody.innerHTML = `<tr><td colspan="3" class="py-3 px-2 text-center text-slate-500 text-[10px]">Failed to load activity</td></tr>`;
    }
}
