...
async function loadRoles() {
    try {
        const response = await fetch('/api/super_admin/people/manage-users/roles', { credentials: 'include' });
        ...
        // Populate edit role dropdown (excluding super_admin)
...
async function loadCompanies() {
    try {
        const response = await fetch('/api/super_admin/people/manage-users/companies', { credentials: 'include' });
...
async function loadAllUsers() {
    try {
        const response = await fetch('/api/super_admin/people/manage-users/users', {
            method: 'POST',
...
// ─── User Table Rendering (centralized createTable) ──────────────────────────
            ...
            headers: [
                ...
                { label: 'Actions', key: 'id', width: '12%', align: 'right', render: (val, row) => {
                    ...
                    return '<div class="flex gap-1 justify-end">' +
                        ...
                        '<button onclick="toggleUserStatus(' + val + ', ' + (active ? 'true' : 'false') + ')" class="px-1.5 py-0.5 bg-white hover:bg-' + (active ? 'rose-100' : 'emerald-100') + ' text-violet-700 text-[9px] rounded border border-slate-300 hover:border-' + (active ? 'rose-300' : 'emerald-300') + ' transition" title="' + (active ? 'Deactivate user' : 'Activate user') + '">' +
...
document.getElementById('editUserForm').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch(`/api/super_admin/people/manage-users/users/${userId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({
