...
async function reloadControls() {
  ...
  if (!container) return;
  container.innerHTML = '<div class="p-6 text-center text-slate-400 text-xs">Loading...</div>';
  let stored = {};
  try {
    const json = await apiJson('/api/tables/controls');
    (json.items || []).forEach(it => { stored[it.tableId] = it.controls || {}; });
  } catch (e) {
    container.innerHTML = '<div class="p-6 text-center text-rose-400 text-xs">Failed to load controls: ' + esc(e.message) + '</div>';
    return;
  }

  let html = '<table class="w-full text-left text-xs">' +
    '<thead><tr class="border-b border-slate-800 bg-slate-800/30">' +
      '<th class="py-2 px-3 text-slate-400 uppercase font-medium">Table</th>' +
      CONTROL_FIELDS.map(f => '<th class="py-2 px-3 text-slate-400 uppercase font-medium text-center">' + f.label + '</th>').join('') +
      '<th class="py-2 px-3 text-slate-400 uppercase font-medium">Actions</th>' +
    '</tr></thead><tbody>';

  if (KNOWN_TABLES.length === 0) {
    html += '<tr><td colspan="' + (CONTROL_FIELDS.length + 2) + '" class="py-4 text-center text-slate-500">No tables defined</td></tr>';
  } else {
    KNOWN_TABLES.forEach(function(t) {
      ...
      html += '<tr class="border-b border-slate-800/60">' +
        '<td class="py-2.5 px-3"><span class="text-white font-medium">' + esc(t.label) + '</span>' +
        '<div class="text-[10px] text-slate-500 font-mono">' + esc(t.id) + '</div></td>' +
        CONTROL_FIELDS.map(f =>
          '<td class="py-2.5 px-3 text-center">' +
            '<input type="checkbox" id="' + esc(t.id) + '_' + f.key + '" ' + (c[f.key] ? 'checked' : '') + ' class="accent-violet-500 h-4 w-4">' +
          '</td>'
        ).join('') +
        '<td class="py-2.5 px-3"><button onclick="saveTable(\'' + esc(t.id) + '\')" class="px-3 py-1 bg-violet-600 hover:bg-violet-500 text-white text-[11px] font-medium rounded transition">Save</button></td>' +
      '</tr>';
    });
  }
